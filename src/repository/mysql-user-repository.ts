...
export class MySqlUserRepository implements UserRepository {
  ...
  async fromToken(token: string): Promise<User> {
    const query  = 'SELECT * FROM `tokens` JOIN `users` ON `users`.`id`=`tokens`.`user_id` WHERE `token`=? LIMIT 1;'
  ...
  async createToken(user: User, token: string): Promise<number> {
    const insertQuery = "INSERT INTO `tokens` (`user_id`, `token`) VALUES (?, ?)";
    return (await this.conn.query(insertQuery, [user.id, token]))[0].affechedRows as number;
  }
}
